 ...
 */
package com.chintan992.xplayer.extractor.mkv;
...
@UnstableApi
public class RobustMatroskaExtractor implements Extractor {

  /**
   * Creates a factory for {@link RobustMatroskaExtractor} instances with the provided {@link
   * SubtitleParser.Factory}.
   */
  public static ExtractorsFactory newFactory(SubtitleParser.Factory subtitleParserFactory) {
    return () -> new Extractor[] {new RobustMatroskaExtractor(subtitleParserFactory)};
  }
  ...
  public static final ExtractorsFactory FACTORY =
      () ->
          new Extractor[] {
            new RobustMatroskaExtractor(SubtitleParser.Factory.UNSUPPORTED, FLAG_EMIT_RAW_SUBTITLE_DATA)
          };

  private static final String TAG = "RobustMatroskaExtractor";
  ...
  /**
   * @deprecated Use {@link #RobustMatroskaExtractor(SubtitleParser.Factory)} instead.
   */
  @Deprecated
  public RobustMatroskaExtractor() {
    this(new DefaultEbmlReader(), FLAG_EMIT_RAW_SUBTITLE_DATA, SubtitleParser.Factory.UNSUPPORTED);
  }

  /**
   * @deprecated Use {@link #RobustMatroskaExtractor(SubtitleParser.Factory, int)} instead.
   */
  @Deprecated
  public RobustMatroskaExtractor(@Flags int flags) {
    this(
   ...
   *     extraction.
   */
  public RobustMatroskaExtractor(SubtitleParser.Factory subtitleParserFactory) {
   ...
   */
  public RobustMatroskaExtractor(SubtitleParser.Factory subtitleParserFactory, @Flags int flags) {
    this(new DefaultEbmlReader(), flags, subtitleParserFactory);
  }

  /* package */ RobustMatroskaExtractor(
  ...
  public final int read(ExtractorInput input, PositionHolder seekPosition) throws IOException {
    ...
    while (continueReading && !haveOutputSample) {
      try {
        ...
        Log.w(TAG, "RobustExtractor: Exception in read loop. inCues=" + inCuesElement + ", seekForCues=" + seekForCues, e);
        if (inCuesElement || seekForCues) {
          Log.w(TAG, "RobustExtractor: Failed to read Cues. Disabling Cues and continuing.", e);
          inCuesElement = false;
          seekForCues = false;
          ...
          if (tracks.size() > 0) {
              // Forced end of input if we can't recover position
              return Extractor.RESULT_END_OF_INPUT;
          }
        }
        // If we are getting constant exceptions even outside cues, we should stop to avoid infinite buffering loop.
        Log.e(TAG, "RobustExtractor: Fatal exception in read.", e); 
  ...
  /** Passes events through to the outer {@link RobustMatroskaExtractor}. */
  private final class InnerEbmlProcessor implements EbmlProcessor {

    @Override
    public @ElementType int getElementType(int id) {
      return RobustMatroskaExtractor.this.getElementType(id);
    }

    @Override
    public boolean isLevel1Element(int id) {
      return RobustMatroskaExtractor.this.isLevel1Element(id);
    }

    @Override
    public void startMasterElement(int id, long contentPosition, long contentSize)
        throws ParserException {
      RobustMatroskaExtractor.this.startMasterElement(id, contentPosition, contentSize);
    }

    @Override
    public void endMasterElement(int id) throws ParserException {
      RobustMatroskaExtractor.this.endMasterElement(id);
    }

    @Override
    public void integerElement(int id, long value) throws ParserException {
      RobustMatroskaExtractor.this.integerElement(id, value);
    }

    @Override
    public void floatElement(int id, double value) throws ParserException {
      RobustMatroskaExtractor.this.floatElement(id, value);
    }

    @Override
    public void stringElement(int id, String value) throws ParserException {
      RobustMatroskaExtractor.this.stringElement(id, value);
    }

    @Override
    public void binaryElement(int id, int contentsSize, ExtractorInput input) throws IOException {
      RobustMatroskaExtractor.this.binaryElement(id, contentsSize, input);
    }
  }
  ...
  protected static final class Track {
     ...
     *
     * <p>It is unfortunately not possible to mark {@link RobustMatroskaExtractor#tracks} as only
